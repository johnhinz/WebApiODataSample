    ...
    public class SecurityLoginLogic : BaseLogic<SecurityLoginPoco>
    {
        ...
        protected override void Verify(SecurityLoginPoco[] pocos)
        {
            ...
            foreach (var poco in pocos)
            {
                ...
                {
                    exceptions.Add(new ValidationException(701, $"Password for SecurityLogin {poco.Id} must be at least 10 characters."));
                }
        ...
        private bool VerifyHash(string plainText, string hashDB)
        {
            const int hashSizeInBytes = 64;

            byte[] hashWithSaltBytes = Convert.FromBase64String(hashDB);
            if (hashWithSaltBytes.Length < hashSizeInBytes)
                return false;
            byte[] saltBytes = new byte[hashWithSaltBytes.Length - hashSizeInBytes];
            for (int i = 0; i < saltBytes.Length; i++)
                saltBytes[i] = hashWithSaltBytes[hashSizeInBytes + i];
            string hashExpected = ComputeHash(plainText, saltBytes);
            return (hashDB == hashExpected);
        }
